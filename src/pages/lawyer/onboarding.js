...
const Onboarding = () => {
  ...
  return (
    <>
      {onboardingType === "individual" && <IndividualOnboarding />}
      {onboardingType === "firm" && <FirmOnboarding />}
      {!onboardingType && (
          ...
          <div className="w-3/4 flex flex-col gap-y-2">
            ...
            {user?.firm?.map((firm) => {
              if (!firm?.onboarding) {
                return (
                  <div
                    className="w-full border rounded-xl flex justify-between p-4 items-center hover:bg-gray-50 cursor-pointer"
                    onClick={() => {
                      setCompanyOnboarding(firm);
                    }}
                  >
                    <div className="flex gap-x-3 items-center">
                      <div>
...
const Step1Individual = ({
  ...
  const [uploadingPracticeCertificate, setUploadingPracticeCertificate] =
    useState(false);
  const [
    uploadingPracticeCertificatePercentage,
    setUploadingCertificatePercentage,
  ] = useState(0);
  ...
  useEffect(() => {
    ...
    for (const key in onboardingDetails) {
      if (
        ...
        (onboardingDetails[key] === "" ||
          !onboardingDetails[key] ||
          onboardingDetails[key]?.length === 0)
      ) {
        completed = false;
        break;
      }
      ...
      <div className="w-full md:w-4/5 h-full flex flex-col bg-legalGreen">
        ...
        <div className="relative w-full h-full -mt-5 md:mt-0">
          <div className="md:hidden absolute p-5 w-full h-full">
            <div className="bg-white opacity-50 w-full h-full"></div>
          </div>
                  ...
                  <div className="mt-1">
                    {(uploadingImage && (
                      <div className="flex gap-x-2 border p-3 rounded-md items-center">
                        ...
                        <div
                          className="h-2 bg-legalGreen transition-all rounded-full"
                          style={{ width: `${uploadingImagePercentage}%` }}
                        ></div>
                      </div>
                    )) ||
                    ...
                    {(uploadingPracticeCertificate && (
                      <div className="flex gap-x-2 border p-3 rounded-md items-center">
                        ...
                        <div
                          className="h-2 bg-legalGreen transition-all rounded-full"
                          style={{
                            width: `${uploadingPracticeCertificatePercentage}%`,
                          }}
                        ></div>
                      </div>
                    )) ||
                      (!uploadingPracticeCertificate &&
                        !completedUploadingCert && (
                          <ImageUpload
                            id="certificate"
                            name="certificate"
                        ...
                        !uploadingPracticeCertificate && (
                          <div className="flex justify-between gap-x-2 border p-2 rounded-md items-center">
                            <div className="flex gap-x-2 items-center">
                              ...
                              <div
                                className="text-legalGreen font-semibold underline"
                                onClick={() => {
                                  window.open(
                                    onboardingDetails.practicingCertificate,
                                    "_blank"
                                  );
                                }}
                              >
...
const Step2Individual = ({
  ...
  useEffect(() => {
    setOnboardingDetails((prevState) => ({
      ...prevState,
      areasOfPractice: selectedCategories.map((cat) => cat._id),
    }));
  }, [selectedCategories]);
                ...
                <div className="flex justify-between">
                  ...
                  <div className="flex flex-col items-center gap-y-2">
                    ...
                    <input
                      ...
                      className="appearance-none h-12 w-12 rounded-md focus:outline-none focus:border-legalYellow focus:ring-legalYellow border border-gray-300 text-[#6F8BA4] checked:bg-legalGreen styledRadio"
                      onChange={() => {
                        navigate("/onboarding?type=individual");
                      }}
                    />
                  </div>
                </div>
                        ...
                        <div className="flex gap-x-2 flex-wrap gap-y-2 z-20 bg-white">
                          {selectedCategories?.map((category, idx) => {
                            return (
                              <div
                                key={idx}
                                className="px-2 py-1 border border-legalYellow text-legalYellow font-semibold rounded-full text-sm flex space-x-1 items-center transition-all ease-in-out"
                              >
                                <div>{category.title}</div>
                                <div
                                  onClick={() => {
