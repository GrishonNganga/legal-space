...
export const MiddleTopNav = ({
  ...
  useEffect(() => {
    ...
    const notViewedNotification = notifications?.find(
  ...
  return (
    <div className="w-full sticky top-0 z-10 flex items-center justify-between bg-gray-50 p-4">
      ...
      <div className="font-extrabold text-[#C4C4C4]">{middleTopNavText}</div>
            ...
            <SlideOver
              ...
              ui={
                <ClientNotificationsUI
                  ...
                  closeNotifications={() => setShowNotification(false)}
                />
              }
            />
          )}
            ...
            <div>
              {notification.stage === "accepted" && (
                <span>
                  ...
                  {notification?.lawyerId?.firstName} has been accepted
                </span>
              )}
              {notification.stage === "rejected" && (
                <span>
                  Your appointment request with{" "}
                  {notification?.lawyerId?.firstName} has been rejected
                </span>
              )}
              {notification.stage === "completed" && (
                <span>
                  Your appointment with {notification?.lawyerId?.firstName} has
                  been closed
                </span>
              )}
            </div>
        ...
        if (!notification?.seen) {
          return (
            <div className="text-sm hover:bg-gray-50 p-2">
              ...
              {notification?.appointmentId?.clientId?.firstName}{" "}
              <span
                className="underline text-blue-400 hover:cursor-pointer"
                onClick={() => {
                  closeNotifications();
                  navigate(
                    `/dashboard/appointments/${notification?.appointmentId?._id}`
                  );
                }}
              >
                View
