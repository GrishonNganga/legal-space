    ...
    setNavigation((prevState) => {
      const updatedNavigation = navigation?.map((nav) => {
        if (nav?.children) {
          ...
          const updatedSubNav = nav?.children?.map((route) => {
            if (
              (nav?.enableNavigation &&
                currentRoute?.includes(routePrefix + route?.href)) ||
              routePrefix + route?.href === currentRoute
            ) {
              matched = true;
          ...
          if (
            (nav?.enableNavigation &&
              currentRoute?.includes(routePrefix + nav?.href)) ||
            routePrefix + nav?.href === currentRoute
          ) {
            matched = true;
                      ...
                      {navigation?.map((item) =>
                        !item.children ? (
                          <div
                            key={item.name}
                            onClick={() => {
                              navigate(routePrefix + item?.href);
                            }}
                          >
                            <div
                            ...
                            >
                              <item.icon
                                className={classNames(
                                  item.current
                                    ? "text-gray-500 text-legalYellow"
                                ...
                                <Disclosure.Panel className="space-y-1">
                                  {item.children.map((subItem) => (
                                    <Disclosure.Button
                                      ...
                                      onClick={() => {
                                        navigate(routePrefix + subItem.href);
                                      }}
                                    >
                                      {subItem.name}
                                    </Disclosure.Button>
  ...
  return (
    <>
      {(!collapsedMenu && (
        ...
        <div
          ref={navRef}
          className="hidden md:flex md:w-16 md:flex-col md:fixed md:inset-y-0 ease-in-out duration-300"
        >
          <div className="flex-1 flex flex-col min-h-0 border-r border-gray-200 bg-white">
            <div className="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
              <div className="mt-5 flex-grow flex flex-col">
                ...
                <div className="group block flex-shrink-0">
                  <div className="flex items-center">
                    ...
                    <div className="ml-3">
                      ...
                      <p className="text-sm fon t-medium text-gray-500 group-hover:text-gray-700">
                        View profile
                      </p>
                    </div>
                  </div>
    ...
    <>
      {open && (
        <div className=" ease-in-out duration-300 shadow-lg">
          ...
          >
            <div className="w-full flex flex-shrink-0 p-4 cursor-pointer">
          ...
          {user?.firm?.length > 0 &&
            user?.firm.map((firm) => {
              return (
                <div
                  className="w-full flex flex-shrink-0 p-4 cursor-pointer"
                  onClick={() => {
                    navigate(`firm/${firm._id}`);
                  }}
                >
                  <div className="w-full flex justify-between items-center group flex-shrink-0">
                    <div className="flex items-center">
