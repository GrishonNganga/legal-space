          ...
          steps.map((step, stepIdx) => (
            <li
              ...
              className={classNames(
                stepIdx !== steps.length - 1 ? "pr-3 sm:pr-4" : "",
                "relative"
              )}
            >
...
export const Button = ({
  ...
  return (
    <>
      <button
        className={`w-full flex justify-center py-3 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 transition-all ${
          type === "secondary"
            ? "bg-legalGreen text-legalYellow"
            : "bg-legalYellow text-legalGray"
        } ${!active && "opacity-50"} ${classNames}`}
        onClick={clicked}
      >
...
export const Spinner = ({ size }) => {
  return (
    <div role="status" className="transition-all">
      <svg
        aria-hidden="true"
        className={`w-5 h-5 font-black text-legalYellow animate-spin ${
    ...
    <>
      <div className="hidden md:flex w-full h-full bg-legalGreen flex-col justify-between">
        <div className="w-full h-full flex justify-center items-center relative overflow-hidden pb-72">
          <div
            className={`absolute w-[343px] h-[343px] border border-gray-500 rounded-full ${
              shimmer && "animate-pulse"
            }`}
          ></div>
          <div
            className={`absolute w-[515px] h-[515px] border border-gray-500 rounded-full ${
              shimmer && "animate-pulse"
            }`}
          ></div>
          <div
            className={`absolute w-[687px] h-[687px] border border-gray-500 rounded-full ${
              shimmer && "animate-pulse"
            }`}
          ></div>
          <div
            className={`absolute w-[859px] h-[859px] border border-gray-500 rounded-full ${
              shimmer && "animate-pulse"
            }`}
          ></div>
          <div
            className={`absolute w-[1031px] h-[1031px] border border-gray-500 rounded-full ${
              shimmer && "animate-pulse"
            }`}
          ></div>
        </div>
      ...
      <div className="w-full h-full flex md:hidden justify-center items-center relative overflow-hidden pb-28">
        <div
          className={`absolute w-[250px] h-[250px] border border-gray-500 rounded-full ${
            shimmer && "animate-pulse"
          }`}
        ></div>
        <div
          className={`absolute w-[350px] h-[350px] border border-gray-500 rounded-full ${
            shimmer && "animate-pulse"
          }`}
        ></div>
        <div
          className={`absolute w-[450px] h-[450px] border border-gray-500 rounded-full ${
            shimmer && "animate-pulse"
          }`}
        ></div>
        <div
          className={`absolute w-[550px] h-[550px] border border-gray-500 rounded-full ${
            shimmer && "animate-pulse"
          }`}
        ></div>
        <div
          className={`absolute w-[650px] h-[650px] border border-gray-500 rounded-full ${
            shimmer && "animate-pulse"
          }`}
        ></div>
      </div>
  ...
  return (
    <>
      {label !== "" && (
        <label
          htmlFor={name}
          className="block text-sm font-medium text-legalBlue pb-2"
        >
          {label}
        </label>
      )}
      ...
      >
        ...
        <div className="fixed inset-0 z-10 overflow-y-auto">
          <div className="flex min-h-full items-center justify-center p-4 text-center sm:items-center sm:p-0">
            ...
            >
              <Dialog.Panel
                className={`relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 w-full sm:max-w-xl md:max-w-2xl ${modalStyle}`}
              >
                <div className="flex justify-between p-5">
                  ...
                  <div>
                    <XMarkIcon
                      className="w-6 h-6 text-gray-500 cursor-pointer hover:text-black transition-all"
                      onClick={() => {
                        setOpen(false);
                      }}
                    />
  ...
  return (
    <>
      {label !== "" && (
        <label
          htmlFor={name}
          className="block text-sm font-medium text-legalBlue"
        >
          {label}
        </label>
      )}
    ...
    <div className="lg:flex lg:h-full lg:flex-col">
      <header className="flex items-center justify-between border-b border-gray-200 py-4 px-6 lg:flex-none">
        <h1 className="text-lg font-semibold text-gray-900">
          <time
            dateTime={currentDate?.toLocaleDateString().replaceAll("/", "-")}
          >
            ...
            {days.map((day, index) => (
              ...
              >
                <time
                  ...
                  className={classNames(
                    day.isToday &&
                      "flex h-6 w-6 items-center justify-center rounded-full bg-legalGreen text-white",
                    day.isSelected
                      ? "flex h-6 w-6 items-center justify-center rounded-full"
                      : "",
